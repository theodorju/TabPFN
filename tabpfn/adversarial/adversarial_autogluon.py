...
for dataset_fn in datasets_fn:
    ...
    train_data = TabularDataset(dataset_name + '_train.csv')
    test_data = TabularDataset(dataset_name + '_test.csv')
    predictor = TabularPredictor(label='label').fit(train_data=train_data, presets="best_quality")

    y_test = np.load(dataset_name + "_y_test.npy")
    predictions = predictor.predict(test_data)
    y_pred = predictions.to_numpy()
    acc = accuracy_score(y_test, y_pred)
    print(acc)

    test_data_modified = TabularDataset(dataset_name + '_test_modified.csv')
